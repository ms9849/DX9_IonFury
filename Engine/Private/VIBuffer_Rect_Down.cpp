...
CVIBuffer_Rect_Down::CVIBuffer_Rect_Down(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CVIBuffer { pGraphic_Device }
{
}

CVIBuffer_Rect_Down::CVIBuffer_Rect_Down(const CVIBuffer_Rect_Down& Prototype)
	: CVIBuffer{ Prototype }
{
}

HRESULT CVIBuffer_Rect_Down::Initialize_Prototype()
{
	m_iNumVertices = 4;
...
HRESULT CVIBuffer_Rect_Down::Initialize(void* pArg)
{
	return S_OK;
}

_bool CVIBuffer_Rect_Down::Picking(CTransform* pTransform, _float3* pOut, _float3 vPos, _float3 vLook)
{
...
CVIBuffer_Rect_Down* CVIBuffer_Rect_Down::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CVIBuffer_Rect_Down* pInstance = new CVIBuffer_Rect_Down(pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CVIBuffer_Rect_Down");
...
CComponent* CVIBuffer_Rect_Down::Clone(void* pArg)
{
	CVIBuffer_Rect_Down* pInstance = new CVIBuffer_Rect_Down(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CVIBuffer_Rect_Down");
		Safe_Release(pInstance);
	}
