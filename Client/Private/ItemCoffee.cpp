...
CItemCoffee::CItemCoffee(LPDIRECT3DDEVICE9 pGraphic_Device) :
    CItem { pGraphic_Device }
{
}

CItemCoffee::CItemCoffee(const CItemCoffee& Prototype) :
	CItem { Prototype }
{
}

HRESULT CItemCoffee::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CItemCoffee::Initialize(void* pArg)
{
...
void CItemCoffee::Priority_Update(_float fTimeDelta)
{
}

void CItemCoffee::Update(_float fTimeDelta)
{
    if (!m_bParabola)
...
void CItemCoffee::Late_Update(_float fTimeDelta)
{
    Compute_CamDistance(m_pTransformCom->Get_State(STATE::POSITION));
    m_pGameInstance->Add_RenderGroup(RENDER::BLEND, this);
}

HRESULT CItemCoffee::Render()
{
...
HRESULT CItemCoffee::Ready_Components()
{
    /* Com_Transform */
    ...
    /* Com_Texture */
    if (FAILED(__super::Add_Component(m_pObjectDesc.iLayerLevel, TEXT("Prototype_Component_Texture_Item_Coffee"),
...
HRESULT CItemCoffee::Begin_RenderState()
{
...
HRESULT CItemCoffee::End_RenderState()
{
...
void CItemCoffee::OnCollision(CGameObject* pDst, COLLISION eColType, _float fTimeDelta)
{
...
CItemCoffee* CItemCoffee::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
    CItemCoffee* pInstance = new CItemCoffee(pGraphic_Device);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Create Failed : Item Coffee");
        Safe_Release(pInstance);
    }
    return pInstance;
}

CGameObject* CItemCoffee::Clone(void* pArg)
{
    CItemCoffee* pInstance = new CItemCoffee(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Clone Failed : Item Coffee");
        Safe_Release(pInstance);
    }
    return pInstance;
}

void CItemCoffee::Free()
{
    __super::Free();
}
