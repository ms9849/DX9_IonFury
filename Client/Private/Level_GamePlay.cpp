...
#pragma endregion
	...
	m_pGameInstance->Check_AABBCollision(TEXT("Layer_Monster"), TEXT("Layer_Map_Objects_Gate"), ENUM_CLASS(LEVEL::GAMEPLAY), fTimeDelta);

	m_pGameInstance->Check_OBBCollision(TEXT("Layer_Map_Objects_OBB_Ride"), TEXT("Layer_Player"), ENUM_CLASS(LEVEL::GAMEPLAY), fTimeDelta);
	m_pGameInstance->Check_OBBCollision(TEXT("Layer_Map_Objects_OBB_Ride"), TEXT("Layer_Monster"), ENUM_CLASS(LEVEL::GAMEPLAY), fTimeDelta);

	m_pGameInstance->Check_AABBCollision(TEXT("Layer_Player"), TEXT("Layer_Spawner"), ENUM_CLASS(LEVEL::GAMEPLAY), fTimeDelta);

	m_pGameInstance->Check_RayToAABBCollision(TEXT("Layer_PlayerBullet"), TEXT("Layer_Map_Objects_AABB"), ENUM_CLASS(LEVEL::GAMEPLAY), fTimeDelta, nullptr);
	...
	if (m_pGameInstance->Key_Down(VK_F3) && CHEAT)
	{
...
HRESULT CLevel_GamePlay::Ready_Layer_UI(const _wstring& strLayerTag)
{
	...
	Desc_UseableItems.iLayerLevelIndex = ENUM_CLASS(LEVEL::GAMEPLAY);
	Desc_UseableItems.strLayerTag = strLayerTag;
	Desc_UseableItems.strFontType = TEXT("Default");

	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::GAMEPLAY), TEXT("Prototype_GameObject_UIUseableItems"),
		ENUM_CLASS(LEVEL::GAMEPLAY), strLayerTag, &Desc_UseableItems)))
		return E_FAIL;

	m_pUIUseableItems = dynamic_cast<CUIUseableItems*>(m_pGameInstance->Find_GameObject_ToLayer(ENUM_CLASS(LEVEL::GAMEPLAY), strLayerTag));
...
HRESULT CLevel_GamePlay::Ready_Layer_Items(const _wstring& strLayerTag)
{
	...
	//	return E_FAIL;
	//
	//if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_GameObject_Item_CardKey"),
	//	ENUM_CLASS(LEVEL::GAMEPLAY), strLayerTag)))
	//	return E_FAIL;
...
HRESULT CLevel_GamePlay::Ready_Layer_Map_Objects_OBB_Ride(const _wstring& strLayerTag)
{
	for (auto& iter : m_ObjectDescs->find(strLayerTag)->second)
	{
		...
		CBoxCollider* pCollider = dynamic_cast<CBoxCollider*>(m_pGameInstance->Get_Component(ENUM_CLASS(iter.iLayerLevel), iter.strLayer, TEXT("Com_BoxCollider"), iter.iObjectID));
		pCollider->Set_Scale({ 1.0f, 1.0f, 1.0f });
	}
